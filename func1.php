...
if(isset($_POST['docsub1'])){
  ...
  $array = [];
  foreach ($rows as $row) {
    if ($row[1] == $demail && $row[2] == $dpass) {
      $array = $row;
      break;
    }
  }
  if(sizeof($array)>0){
  $_SESSION['dname']=$array[0];
  $_SESSION['demail']=$array[1];
  $_SESSION['dspecialist']=$array[3];
  $_SESSION['dfee']=$array[4];
  $_SESSION['dregistered_on']=$array[5];
